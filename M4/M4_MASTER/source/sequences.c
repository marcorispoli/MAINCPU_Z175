...
void rxNotifyData(unsigned char type, unsigned char code)
{
    ...
    int sample_time = 0;
    Ser422ReadRegister(_REGID(PR190_NSAMPLES_TIME),10,&PCB190_CONTEST);
    sample_time = (float) _DEVREGL(PR190_NSAMPLES_TIME,PCB190_CONTEST);
    if(sample_time<PCB190_NSAMPLES_TIME){
        addr = _DEVADDR(PR190_SAMPLES_TIME);
        banco = _DEVBANCO(PR190_SAMPLES_TIME);
        for(i=0;i<sample_time;i++,addr++){
            if(i>=sizeof(ts)) break;
            unsigned char cc;
