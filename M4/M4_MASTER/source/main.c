...
{ 
   ...
   { _IDTASK(TOMO_AEC_RX_TASK),         tomo_aec_rx_task,       1500,   _PRIOTASK(TOMO_AEC_RX_TASK ),   "tomo_aec_rx_task",  0,                      0,      0 },
...
void startupDigitalGantry(void){
  _task_id tid;
   ...
   printf("STARTUP GANTRY DIGITAL MODEL\n");

    // Partenza sequenze raggi nella versione digitale
   tid= _task_create(0,_IDTASK(STD_RX_TASK),0);
   tid= _task_create(0,_IDTASK(STD_RX_AEC_TASK),0);
   tid= _task_create(0,_IDTASK(STD_RX_AE_TASK),0);
   tid= _task_create(0,_IDTASK(TOMO_RX_TASK),0);
   tid= _task_create(0,_IDTASK(TOMO_AEC_RX_TASK),0);


   //   Apertura driver PCB244: il driver NON deve attendere la connessione
   tid = _task_create(0,_IDTASK(PCB244),(uint32_t) NULL);
...
void mainPrintHardwareConfig(void){
    ...
    else if(generalConfiguration.gantryCfg.detectorType == DETECTOR_SOLO) printf("DETECTOR: %s\n",DETECTOR_SOLO_STR);
    else if(generalConfiguration.gantryCfg.detectorType == DETECTOR_FDI) printf("DETECTOR: %s\n",DETECTOR_FDI_STR);
    else if(generalConfiguration.gantryCfg.detectorType == DETECTOR_LMAMV2) printf("DETECTOR: %s\n",DETECTOR_LMAMV2_STR);
    else if(generalConfiguration.gantryCfg.detectorType == DETECTOR_SMAM) printf("DETECTOR: %s\n",DETECTOR_SMAM_STR);
    else if(generalConfiguration.gantryCfg.detectorType == DETECTOR_SMAMV2) printf("DETECTOR: %s\n",DETECTOR_SMAMV2_STR);
    else if(generalConfiguration.gantryCfg.detectorType == DETECTOR_FDIV2) printf("DETECTOR: %s\n",DETECTOR_FDIV2_STR);
    else if(generalConfiguration.gantryCfg.detectorType == DETECTOR_SCREENPLUSV2) printf("DETECTOR: %s\n",DETECTOR_SCREENPLUSV2_STR);
    else printf("INVALID DETECTOR:%d\n",generalConfiguration.gantryCfg.detectorType);
