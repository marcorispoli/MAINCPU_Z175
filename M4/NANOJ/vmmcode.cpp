...
void user()
{
	...
	for(samples=0; samples<32; samples++){
		od_write(0x2500, samples, 0 );
		yield();	
	}	
	samples = 0;
	...
	// Attende che sia disattivo, per evitare partenze false (EXP-WIN basso per AEC) 
	while((In.ExpWin & 0x10000) != 0x00000) yield();
	sleep(50);
	
	// Attende fronte di attivazione Input 
