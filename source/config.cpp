...
bool Config::openSysCfg(void)
{
    ...
    sys.biopsyType = BYM_STANDARD_DEVICE;       // Modalit‡ standard di default
...
bool Config::setTomoSpeedMode(QString tomoModeStr, int console_id)
{
    ...
    // Verifica la modalit√  corrente 1F .. to 5F e salva eventualmente la nuova modalit√
    if(tomoModeStr == "4F"){
    ...
    }else if(tomoModeStr == "5F"){
        trxConfig.tomo_mode = _TOMO_MODE_5F;
    }else return false;
...
// Configuration file auto generante se mancante
bool Config::openUserCfg(void)
{
    ...
    userCnf.enableCheckAccessorio = true;
    userCnf.enableComboCheckAccessorio = true; // Non usato
    userCnf.enable2DCheckAccessorio = true;    // Non usato
    userCnf.enable3DCheckAccessorio = false;   // Viene attivato solo se montato almeno una volta
    userCnf.enablePbCheckAccessorio = false;   // Viene impostato in maniera fissa: non pi˘ applicabile
    ...
    userCnf.tubeFileName="TEMPLATE_XM1016THI";
    ...
    while(1)
    {
        ...
        }else if(dati.at(0)=="ACCESSORIO_PAZIENTE_COMBO"){
            if(dati.at(1)=="1") userCnf.enableComboCheckAccessorio = TRUE;
            else userCnf.enableComboCheckAccessorio = FALSE;
        }else if(dati.at(0)=="ACCESSORIO_PAZIENTE_2D"){
            if(dati.at(1)=="1") userCnf.enable2DCheckAccessorio = TRUE;
            else userCnf.enable2DCheckAccessorio = FALSE;
        }else if(dati.at(0)=="ACCESSORIO_PAZIENTE_3D"){
            if(dati.at(1)=="1") userCnf.enable3DCheckAccessorio = TRUE;
            else userCnf.enable3DCheckAccessorio = FALSE;
        }else if(dati.at(0)=="ACCESSORIO_PAZIENTE_PB"){
            //if(dati.at(1)=="1") userCnf.enablePbCheckAccessorio = TRUE;
            //else userCnf.enablePbCheckAccessorio = FALSE;
...
bool Config::saveUserCfg(void)
{
    ...
    if(userCnf.enableComboCheckAccessorio) file.write("<ACCESSORIO_PAZIENTE_COMBO,1>    // opzione verifica accessorio su Combo\n");
    else file.write("<ACCESSORIO_PAZIENTE_COMBO,0>    // opzione verifica accessorio su Combo\n");

    if(userCnf.enable2DCheckAccessorio) file.write("<ACCESSORIO_PAZIENTE_2D,1>    // opzione verifica accessorio su esposizione 2D\n");
    else file.write("<ACCESSORIO_PAZIENTE_2D,0>    // opzione verifica accessorio su esposizione 2D\n");

    if(userCnf.enable3DCheckAccessorio) file.write("<ACCESSORIO_PAZIENTE_3D,1>    // opzione verifica accessorio su esposizione 3D\n");
    else file.write("<ACCESSORIO_PAZIENTE_3D,0>    // opzione verifica accessorio su esposizione 3D\n");

    if(userCnf.enablePbCheckAccessorio) file.write("<ACCESSORIO_PAZIENTE_PB,1>    // opzione verifica accessorio piombo\n");
    else file.write("<ACCESSORIO_PAZIENTE_PB,0>    // opzione verifica accessorio piombo");
...
QString Config::getTomoFilename(void){
    // Lettura file di configurazione Tomo relativo alla modalit‡ selezionata
    if(trxConfig.tomo_mode==_TOMO_MODE_5F)
        return QString(TOMO_FILE_CFG).arg("5F");
    else if(trxConfig.tomo_mode==_TOMO_MODE_4F)
...
bool Config::checkPackage(void)
{
    ...
    revisionError.append(QString("FW190 [%1]: -> %2").arg(swConf.rv190).arg(rv190));
...
void Config::awsConnectionHandler(bool stat)
{
    ...
    if(awsConnected){
        ApplicationDatabase.setData(_DB_PCAWS_CONNECTION,(unsigned char) 1);
        PRINT("PC SERVER CONNECTED");
    } else{
        PRINT("PC SERVER DISCONNECTED");
        ApplicationDatabase.setData(_DB_PCAWS_CONNECTION,(unsigned char) 0);
    }
...
void Config::masterRequestAwsPowerOff(void){
    ...
    // Invia comando di reboot allo slave
    protoConsole frame(1,false);
    //configMasterTcp->txData(frame.cmdToQByteArray(SLAVE_EXECUTE_PC_POWEROFF), 5000);
