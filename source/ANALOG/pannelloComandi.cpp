...
//    profileLabel = new GLabel(this,QRectF(100,135,150,30),font,QColor(_W_TEXT)," ---",Qt::AlignLeft);
    ...
    // Stringhe di dose e mAs effettivi ultima esposizione
    font.setPointSize(20);
    font.setStretch(50);
    ...
    font.setPointSize(20);
    font.setStretch(50);
    mAsXLabel = new GLabel(this,QRectF(337,321,100,20),font,QColor(_W_TECH_MODE),"X mAs: ----",Qt::AlignLeft);
...
void pannelloComandi::mousePressEvent(QGraphicsSceneMouseEvent* event)
{
    ...
    if((mouse.x()>=185)&&(mouse.x()<=308)&&(mouse.y()>=116)&&(mouse.y()<=260)){
        ApplicationDatabase.setData(_DB_CALLBACKS,(int) CALLBACK_COMANDI_PROJ_SELECTION ,DBase::_DB_FORCE_SGN);
 ...
 * Impostazione Campo filtri
 _____________________________________________________________________________________________________________________________________ */
void pannelloComandi::setFilterField(int filtro, bool automode){
    ...
    if(automode) autoFilterLabel->setPlainText(QString("A").toAscii().data());
...
void pannelloComandi::valueChanged(int index,int opt)
{
    ...
    case _DB_XDUDOSE:
        if(ApplicationDatabase.getDataI(index)==0) doseLabel->setPlainText(QString("AGD: ----").toAscii().data());
        else   doseLabel->setPlainText(QString("AGD: %1 uG").arg(((float)ApplicationDatabase.getDataI(index))/10).toAscii().data());
        doseLabel->update();
        break;
