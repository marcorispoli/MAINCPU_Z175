...
void pannelloSpessore::updateSpessore(){
    ...
    if(ApplicationDatabase.getDataI(_DB_SPESSORE) == 0) spessoreLabel->setPlainText(QString("---").toAscii().data());
    else spessoreLabel->setPlainText(QString("%1").arg(ApplicationDatabase.getDataI(_DB_SPESSORE)).toAscii().data());
