...
void AnalogCalibPageOpen::initManualExposure(void){
    ...
    ui->manualPanelFrame->setGeometry(160,60,506,351);
...
void AnalogCalibPageOpen::manualExposureGuiNotify(unsigned char id, unsigned char mcccode, QByteArray data)
{
    ...
    case MCC_XRAY_ANALOG_MANUAL:
        ...
        if(data.at(0)){
            ...
            int vmean = ((int) data[13]+ 256 * (int) data[14]);
            int mas = ((int) data[1]+ 256 * (int) data[2]);
            int tpulse = ((int) data[17]+ 256 * (int) data[18]);
            float imean = mas * 100 / tpulse;

            ApplicationDatabase.setData(_DB_MANUAL_KVREAD,(int) (vmean),0);
            ApplicationDatabase.setData(_DB_MANUAL_PULSE_TIME,(int) (tpulse),0);
            ApplicationDatabase.setData(_DB_MANUAL_IAREAD,(int) (imean*10),0);
            ApplicationDatabase.setData(_DB_MANUAL_MASREAD,(int) ((int) data[1]+ 256 * (int) data[2]));
