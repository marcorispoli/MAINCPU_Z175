...
bool AnalogCalibPageOpen::getDetectorCalibrationReady(unsigned char opt){
    ...
    if(ApplicationDatabase.getDataU(_DB_AWS_CONNECTION)){
        if(ApplicationDatabase.getDataU(_DB_READY_EXPOSURE)) ready_stat|=4;
    }else ready_stat|=4;

    //if(pCompressore->isValidPad()) ready=false;
    if(pPotter->getPotId()==POTTER_2D) ready_stat|=1; // Potter 2D
    if(pPotter->getCassettePresence()) ready_stat|=2; // Cassetta
    ApplicationDatabase.setData(_DB_READY_STAT,ready_stat,opt);
    if(ready_stat == 5) return true;
    return false;
}
...
void AnalogCalibPageOpen::detectorValueChanged(int index,int opt)
{
    ...
    case _DB_DETECTOR_OFSET: // Il valore ricevuto è la somma di quattro campionamenti sul canale 25x
        if(ApplicationDatabase.getDataI(_DB_DETECTOR_OFSET)) ui->ofsRadLabel->setText(QString("OFS:%1").arg((float) ApplicationDatabase.getDataI(_DB_DETECTOR_OFSET)/100));
    ...
    case _DB_AWS_CONNECTION:
        ...
        break;
    case _DB_READY_STAT:
        if(ApplicationDatabase.getDataI(index) & 0x4)  ui->warningPC->hide();
        else ui->warningPC->show();

        if(ApplicationDatabase.getDataI(index) & 0x1){
            // Potter presente
            ui->warningPotter->hide();
            if(ApplicationDatabase.getDataI(index) & 0x2) ui->warningCassette->show();
            else ui->warningCassette->hide();
        }else{
            // Potter 2D assente
...
void AnalogCalibPageOpen::startDetectorCalibrationXraySequence(void){
    ...
    data[6] =  50 | 0x80; // 500ms massimi per ogni impulso: usa il timer corto da 10ms
