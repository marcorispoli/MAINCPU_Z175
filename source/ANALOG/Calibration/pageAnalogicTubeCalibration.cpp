...
void AnalogCalibPageOpen::exitTubeCalibration(void){
    ...
    disconnect(pConsole,SIGNAL(raggiDataSgn(QByteArray)),this,SLOT(raggiTubeDataSlot(QByteArray)));
...
void AnalogCalibPageOpen::initTubeCalibration(void){
    ...
    connect(pConsole,SIGNAL(raggiDataSgn(QByteArray)),this,SLOT(raggiTubeDataSlot(QByteArray)),Qt::UniqueConnection);
...
void AnalogCalibPageOpen::raggiTubeDataSlot(QByteArray data)
{

        int tpls,mas,imean;

        unsigned char raw_vmean = data.at(V_MEAN);
        float vmean = pGeneratore->convertPcb190Kv(raw_vmean);
        ...
        ApplicationDatabase.setData(_DB_CALIB_TUBE_DKVR, (int) (vmean*10));
        ApplicationDatabase.setData(_DB_CALIB_TUBE_IAR, (int) (imean));
        ApplicationDatabase.setData(_DB_CALIB_TUBE_DMASR, (int) (mas*10/50));

        // Comunica i dati al software:
        // Al PC bisogna mandare:
        // 0, V-MEAN RAW, VMEAN(kV), IMEAN
        pToConsole->fineRaggiAnalogCalibTube(0,raw_vmean, (int) (vmean*10.), imean*10);
        ApplicationDatabase.setData(_DB_XRAY_SYM,(unsigned char) 0, DBase::_DB_FORCE_SGN);
        io->setXrayLamp(false);
...
void AnalogCalibPageOpen::tubeGuiNotify(unsigned char id, unsigned char mcccode, QByteArray rxdata)
{
    ...
    case MCC_XRAY_ANALOG_CALIB_TUBE: // Fine raggi
        ...
        if(rxdata.at(0)){
            ...
            pToConsole->fineRaggiAnalogCalibTube(rxdata.at(0),0,0,0);
        }
