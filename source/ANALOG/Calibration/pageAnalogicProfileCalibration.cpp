...
void AnalogCalibPageOpen::setProfileData(void){
    ...
    if(pGeneratore->selectProfile(pc_selected_profile_index)!=null){
        ApplicationDatabase.setData(_DB_CALIB_PROFILE_NAME, pGeneratore->getCurrentProfilePtr()->symbolicName, DBase::_DB_FORCE_SGN);
    }else{
...
void AnalogCalibPageOpen::profileGuiNotify(unsigned char id, unsigned char mcccode, QByteArray rxdata)
{
    ...
    case MCC_XRAY_ANALOG_CALIB_PROFILE: // Fine raggi
        ...
        //pToConsole->fineRaggiAnalogCalibProfile(data);
    ...
    case MCC_XRAY_ANALOG_REQ_AEC_PULSE:
        ...
        int plog = rxdata[0] + 256*rxdata[1];
        int rad = rxdata[2] + 256*rxdata[3];
        int rad5 = rxdata[4] + 256*rxdata[5];
        int rad25 = rxdata[6] + 256*rxdata[7];


        ApplicationDatabase.setData(_DB_CALIB_PROFILE_PLOG, (int)  plog, DBase::_DB_FORCE_SGN);
        ApplicationDatabase.setData(_DB_CALIB_PROFILE_RAD, (int)  rad, DBase::_DB_FORCE_SGN);
        ...
        int dmas;
        int pulses;
        int error = pGeneratore->getAecData(plog,ANALOG_FILTRO_FISSO,5,ANALOG_TECH_PROFILE_STD,&filtro,&kV,&dmas,&pulses);

        pGeneratore->setkV(kV);
        pGeneratore->setmAs((float) dmas/10);
        ...
        data[18] = (unsigned char) (pulses);
        data[19] = (unsigned char) (pulses>>8);
