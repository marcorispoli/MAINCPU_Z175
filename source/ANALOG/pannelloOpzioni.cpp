...
void pannelloOpzioni::mousePressEvent(QGraphicsSceneMouseEvent* event)
{
    ...
    if(mouse.y()<=107){        
        if((mouse.x()>=140)&&(mouse.x()<=200)){
            ...
            ApplicationDatabase.setData(_DB_FILTER_MODE,(int) 0,DBase::_DB_NO_CHG_SGN);
    ...
    if(mouse.y()<=190){
        ...
        if((mouse.x()>=140)&&(mouse.x()<=200)){
            analog_conf_changed = true;
            ApplicationDatabase.setData(_DB_FILTER_MODE,(int) 0,DBase::_DB_NO_CHG_SGN);
            ApplicationDatabase.setData(_DB_SELECTED_FILTER,(int) Collimatore::FILTRO_Mo);
        }else if((mouse.x()>=214)&&(mouse.x()<=273)){
            analog_conf_changed = true;
            ApplicationDatabase.setData(_DB_FILTER_MODE,(int) 0,DBase::_DB_NO_CHG_SGN);
            ApplicationDatabase.setData(_DB_SELECTED_FILTER,(int) Collimatore::FILTRO_Rh);
        }else if((mouse.x()>=287)&&(mouse.x()<=347)){
            analog_conf_changed = true;
            ApplicationDatabase.setData(_DB_FILTER_MODE,(int) 1);
...
void pannelloOpzioni::open(){
    ...
    if(ApplicationDatabase.getDataI(_DB_FILTER_MODE) == 1) // Filtro automatico (quindi ci sono almeno due filtri configurati)
    {
...
void pannelloOpzioni::valueChanged(int index,int opt)
{
    ...
    case _DB_SELECTED_FILTER:
        if(ApplicationDatabase.getDataI(_DB_FILTER_MODE)==1) selButtonFilterModePix->setPos(SEL_FILTERAUTO_POS);
        ...
        if(isMaster){
            ...
            pConfig->analogCnf.auto_filtro_mode = ApplicationDatabase.getDataI(_DB_FILTER_MODE);
    ...
    case _DB_OD:
        ...
        // Aggiorna i dati del profilo in configurazione
        if(isMaster){
            pGeneratore->getCurrentProfilePtr()->odindex = ApplicationDatabase.getDataI(_DB_OD);
            pGeneratore->getCurrentProfilePtr()->technic = ApplicationDatabase.getDataI(_DB_TECHNIC);
            setExpositionRange(DBase::_DB_NO_CHG_SGN);
        }
        break;
...
void pannelloOpzioni::stepProfile(int dir){
    profileCnf_Str* profilePtr;
    if(!isMaster) return;

    if(dir==1) profilePtr = pGeneratore->getNextProfilePtr();
    else profilePtr = pGeneratore->getPrevProfilePtr();
    ...
    ApplicationDatabase.setData(_DB_PROFILE_INDEX,(int) profilePtr->odindex ,DBase::_DB_FORCE_SGN|DBase::_DB_NO_CHG_SGN);
    ...
    if(profilePtr->plateType == ANALOG_PLATE_FILM){
        ...
        ApplicationDatabase.setData(_DB_TECHNIC,(int) ANALOG_TECH_PROFILE_STD ,DBase::_DB_FORCE_SGN|DBase::_DB_NO_CHG_SGN);
