...
bool Collimatore::updateColli(void)
{
    ...
    }else
    {
        ...
        // In modalitÃ  non operativa utilizza la colimazione OPEN
...
unsigned char Collimatore::getDynamicColliTargetL(float angolo, float dispersione, unsigned char nRef, float footprint){
    ...
    float  NTARGET = nRef - LF * (dispersione+footprint) / (rC * (FC + CD)); // Target N per apertura a 0Â° per avere le lame alla dispersione voluta
...
bool Collimatore::readConfigFile(void)
{
    ...
    // Nel caso la Tomo non sia correttamente configurata verrÃ  impostato il default
