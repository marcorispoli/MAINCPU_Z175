...
void calibfilter::initPage(void){
    ...
    if(isMaster){
        ...
        // Rimuove lo specchio se necessario
        pCollimatore->setMirror(Collimatore::MIRROR_HOME);
...
void calibfilter::valueChanged(int index,int opt)
{
    ...
    switch(index){
        ...
        break;
        case _DB_SERVICE4_INT:  // Pulsanti di modifica posizione per filtro selezionato
...
void calibfilter::guiNotifySlot(unsigned char id, unsigned char mcccode, QByteArray buffer)
{
    ...
    case MCC_SET_FILTRO:
        ...
        }else
        {
            ...
            //ApplicationDatabase.setData(_DB_SERVICE3_INT,(int) buffer.at(2)); // Posizione fisica filtro
            ApplicationDatabase.setData(_DB_SERVICE3_INT,(int) pCollimatore->colliConf.filterPos[buffer.at(1)]); // Posizione fisica filtro
...
void calibfilter::onAdjustLeft(void){
    ApplicationDatabase.setData(_DB_SERVICE4_INT,(int) 2);
}
...
void calibfilter::onStoreButton(void){
    ApplicationDatabase.setData(_DB_SERVICE5_INT,(int) 1);
}
