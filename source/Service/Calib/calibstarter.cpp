...
void calibstarter::valueChanged(int index,int opt)
{
    ...
    case _DB_SERVICE10_INT:
        ApplicationDatabase.setData(_DB_SERVICE10_INT,(int) 0,DBase::_DB_NO_CHG_SGN|DBase::_DB_NO_ECHO);

        if(isMaster) activateTest();
        break;

    case _DB_SERVICE11_INT:
        ApplicationDatabase.setData(_DB_SERVICE11_INT,(int) 0,DBase::_DB_NO_CHG_SGN|DBase::_DB_NO_ECHO);
...
void calibstarter::onStartButton(void){
    ...
    timerDisable = startTimer(1000);
    ApplicationDatabase.setData(_DB_SERVICE10_INT,(int) 1);
}

void calibstarter::onStoreButton(void){
    if(timerDisable) return;
    timerDisable = startTimer(1000);
    ApplicationDatabase.setData(_DB_SERVICE11_INT,(int) 1);
}
