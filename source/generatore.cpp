...
bool Generatore::openTube(QString tubeDir)
{
    ...
    // Impostazione della modalit‡ di sistema di gestione del Tubo ad Aria
    if(tubeDir.contains("XK1016T"))   airTube=true;
    else airTube = false;

    // Se la configurazione di sistema Ë gi‡ stata inviata allora occorre riaggiornarla
...
int Generatore::manualXraySequence(void)
{
    ...
    // Verifica se la modalit‡ manuale Ë attiva
    if(!manualMode) return -1;

    // Verifica se un tubo Ë stato selezionato
