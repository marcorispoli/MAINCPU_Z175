...
{
    // Solo per macchine Analogiche
    if(pConfig->sys.gantryModel==GANTRY_MODEL_ANALOG) return;

    // Se il sistema si trova in calibrazione nn genera allarmi sul cambio pad
    if(!pConsole->isOperatingMode()) {
        PageAlarms::activateNewAlarm(_DB_ALLARMI_ALR_PAD,0);
        return;
    }

    // Verifica condizioni a studio aperto
    if(ApplicationDatabase.getDataU(_DB_STUDY_STAT)!=_CLOSED_STUDY_STATUS){
        switch(comprPad)
        {
            case PAD_UNLOCKED: PageAlarms::addNewAlarm(_DB_ALLARMI_ALR_PAD,_ALR_UNLOCK_COMPR);
                break;
            case PAD_UNMOUNTED: PageAlarms::addNewAlarm(_DB_ALLARMI_ALR_PAD,_ALR_UNLOCK_PAD);
                break;
            case POTTER_DISCONNECTED: PageAlarms::addNewAlarm(_DB_ALLARMI_ALR_PAD,_ALR_UNLOCK_POTTER);
            break;
            case PAD_ND: PageAlarms::addNewAlarm(_DB_ALLARMI_ALR_PAD,_ALR_WRONG_PAD);
                break;
            default:
                PageAlarms::activateNewAlarm(_DB_ALLARMI_ALR_PAD,0);
        }
